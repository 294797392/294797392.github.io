...
// 释放array数据结构宏
// @arr:                需要释放的array
// @func_release_elem   释放array内部元素的函数指针. 
//                      签名为 void (*ptr_func_release_elem)(void*)
//-------------------------------------------------------------------------------------
#define RELEASE_ARRAY(arr, ptr_func_release_elem) \
{ \
    if(ptr_func_release_elem) { \
        void (*ptr_func)(void*); ptr_func = ptr_func_release_elem; \
        void* ptr; while ((ptr = array_dequeue(arr)) != NULL) ptr_func(ptr); \
    } \
    release_array(arr); \
}
